...
def process_one_record():
	...
	try:
		...
		for process_record in list_body_to_process:		
			...
			reply['request_body']=body_pass
			

			response1 = requests.post(creating_url, headers=headers, json=body_pass)
...
def update_serial_no_records():
	...
	try:
		query = "UPDATE `tabSerial No` SET `custom_marked_in_avdm`=1 WHERE `name` IN {}".format(flat_list2)
		...
		frappe.sendmail(
			...
			message="Line 259 avdm.py <br>{} <br>{}".format(str(e),str(serial_no_list))
		)
