...
frappe.ui.form.on('Bank Reconciliation Tool Mantra', {
	...
	refresh: function(frm) {
		...
		frm.page.add_menu_item(__("Auto Reconcile"), function () {
			frappe.confirm(
				...
				() => {
					frappe.call({
						method: "mantra_dev.mantra_dev.doctype.bank_reconciliation_tool_mantra.bank_reconciliation_tool_mantra.auto_reconcile_vouchers",
						args: {
	...
	route_to_bank_statement_import(frm) {
		...
		frappe.call({
			method:
				"mantra_dev.mantra_dev.doctype.bank_reconciliation_tool_mantra.bank_reconciliation_tool_mantra.upload_bank_statement",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
