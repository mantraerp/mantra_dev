...
class RepostItemValuation(Document):
	...
	def restart_reposting(self):
		...
		self.clear_attachment()
		self.db_update()
		frappe.enqueue(repost,queue='long',job_name="Reposting",timeout=100000,doc=self)
...
def repost_entries():
	...
	for row in riv_entries:
		...
		if doc.status in ("Queued", "In Progress"):
			# repost(doc)
			frappe.enqueue(repost,queue='long',job_name="Repost {}".format(row.name),timeout=5000,doc=doc)
...
def get_repost_item_valuation_entries():
	query = "SELECT name from `tabRepost Item Valuation` WHERE status in ('Queued', 'In Progress') and creation <= '{}' and posting_date >= '{}' and docstatus = 1 ORDER BY timestamp(posting_date, posting_time) asc, creation asc, status asc limit 500".format(now(),'2024-08-01')
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	if len(total_repost_entries)==0:
		get_repost_item_valuation_entries_fail()

	return total_repost_entries

# @frappe.whitelist(allow_guest=True)
def get_repost_item_valuation_entries_fail():
	# frappe.msgprint('test')
	query = "SELECT name from `tabRepost Item Valuation` WHERE status in ('Failed') and creation <= '{}' and posting_date >= '{}' and docstatus = 1 ORDER BY timestamp(posting_date, posting_time) asc, creation asc, status asc limit 500".format(now(),'2024-10-01')
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	for row in total_repost_entries:
		frappe.enqueue(change_fail_status,queue='long',job_name="Repost status change {}".format(row.name),timeout=5000,doc_name=row.name)

	return 'Entries status change'


@frappe.whitelist(allow_guest=True)
def get_repost_item_valuation_entries_dynamic_query_test(query):
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	return total_repost_entries
	for row in total_repost_entries:
		frappe.enqueue(change_fail_status,queue='long',job_name="Repost status change {}".format(row.name),timeout=5000,doc_name=row.name)
...
@frappe.whitelist(allow_guest=True)
def get_repost_item_valuation_entries_dynamic_query(query):
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	for row in total_repost_entries:
		frappe.enqueue(change_fail_status,queue='long',job_name="Repost status change {}".format(row.name),timeout=5000,doc_name=row.name)
...
@frappe.whitelist(allow_guest=True)
def process_entry_dynamic_query_test(query):
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	return total_repost_entries
	for row in total_repost_entries:
		process_one_entry(row.name)

	return True

@frappe.whitelist(allow_guest=True)
def process_entry_dynamic_query(query):
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	for row in total_repost_entries:
		process_one_entry(row.name)
...
def change_fail_status(doc_name):
	doc = frappe.get_doc("Repost Item Valuation", doc_name)
	if doc.status in ("Failed"):
		query = "UPDATE `tabRepost Item Valuation` SET status='Queued' WHERE name='{}'".format(doc_name)
		frappe.db.sql(query)
...
@frappe.whitelist(allow_guest=True)
def get_all_fail_status_entry(allow_guest=True):
	query = "SELECT name from `tabRepost Item Valuation` WHERE status in ('Failed') and creation <= '{}' and posting_date >= '{}' and docstatus = 1 ORDER BY timestamp(posting_date, posting_time) asc, creation asc, status asc limit 500".format(now(),'2024-10-01')
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	return total_repost_entries
...
@frappe.whitelist(allow_guest=True)
def process_one_fail_entry(allow_guest=True):
	query = "SELECT name from `tabRepost Item Valuation` WHERE status in ('Failed') and creation <= '{}' and posting_date >= '{}' and docstatus = 1 ORDER BY timestamp(posting_date, posting_time) asc, creation asc, status asc limit 1".format(now(),'2024-10-01')
	total_repost_entries = frappe.db.sql(query,as_dict=1)
	for row in total_repost_entries:
		process_one_entry(row.name)
...
@frappe.whitelist(allow_guest=True)
def process_one_entry(doc_name):
	...
	doc = frappe.get_doc("Repost Item Valuation", doc_name)
	if doc.status in ("Queued", "In Progress","Failed"):
		frappe.enqueue(repost,queue='long',job_name="Repost {}".format(doc_name),timeout=5000,doc=doc)
		doc.deduplicate_similar_repost()
		return "Record process"

	return "Record stauts not in queue, in process or failed."


@frappe.whitelist(allow_guest=True)
def get_all_background_jobs(allow_guest=True):
...
def check_same_background_job(allow_guest=True):
	...
	final_status = False
	for index, job in enumerate(all_background_jobs['mantratecerpstage.com']):
		if str(job).startswith("<function Repost"):
			final_status = True
			break
...
def get_repost_item_valuation_entries2():
