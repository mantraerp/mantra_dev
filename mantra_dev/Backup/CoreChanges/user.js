	...
	before_save(frm){
		if(frm.doc.new_password){
			let COMMON_PASSWORDS = [
				...
				"welcome", "monkey", "login", "football", "mantra@123", "mefron@123"
			];
			let FORBIDDEN_KEYWORDS = ["mantra", "mefron", "mewruk"];

			for (let keyword of FORBIDDEN_KEYWORDS) {
				if (frm.doc.new_password.toLowerCase().includes(keyword)) {
					frappe.throw(`Password cannot contain the word '${keyword}'. Please choose a more secure password.`);
					return false;
				}
			}
			if (COMMON_PASSWORDS.includes(frm.doc.new_password.toLowerCase())) {
				frappe.throw("Password is too common. Please choose a more secure password.");
				return false;
			}

			
			const password = frm.doc.new_password;
			const validations = [
				/[A-Z]/.test(password), 
				/[a-z]/.test(password), 
				/[0-9]/.test(password), 
			...
			if (validations.includes(false)) {
				frappe.throw(("Password must contain at least 12 characters, including an uppercase letter, a lowercase letter, a number, and a special character."));
				return false;
			}
		}
	},	
