...
# apps/erpnext/erpnext/utilities
# path to change file
...
###################################################Change in core###############################
	if to_doctype == "Payment Entry":
		meta = frappe.get_meta('Payment Entry')
		field = meta.get_field('mode_of_payment')
		#To check in doctype defaul value is set or not
		if field and field.default:
			x = field.default
		else:
			frappe.throw("Default Mode of Payment does not exists")
			return

		doc1 = frappe.get_doc("Mode of Payment",x)
		if doc1.accounts:
			for row in doc1.accounts:
				if row.default_account:
					...
					if set_acc:
						...
						frappe.throw("There is no Bank Account releted to this internal account.")
						return
				else:
					frappe.throw("Bank Account does not exists for Default Mode of Payment")
					return
		else:
			frappe.throw(f"Account not set for this mode of payment({x}).")
			return
############################################################################################################
