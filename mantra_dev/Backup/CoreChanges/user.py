...
def update_password(
    ...
    "welcome", "monkey", "login", "football", "mantra@123", "mefron@123, mewruk@123"
	]
	FORBIDDEN_KEYWORDS = ["mantra", "mefron", "mewruk"]

	    # Check if password contains forbidden keywords
	for keyword in FORBIDDEN_KEYWORDS:
		if keyword in new_password.lower():
			frappe.throw(f"Password cannot contain the word '{keyword}'. Please choose a more secure password.")
			return False
	...
	if not (len(new_password) >= 12 and re.search(r'[A-Z]', new_password) and re.search(r'[a-z]', new_password) and re.search(r'[0-9]', new_password) and re.search(r'[\W_]', new_password)):
